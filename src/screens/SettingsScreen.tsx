...
const SettingsScreen = ({ navigation }) => {
  ...
  const getCurrencyName = (symbol: string) => {
    return CURRENCIES.find(c => c.symbol === symbol)?.name || '';
  };
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50">
      ...
      <ScrollView className="flex-1 px-6 pt-6" keyboardShouldPersistTaps="handled">
        ...
        <View className="bg-white rounded-2xl border border-slate-100 mb-4 overflow-hidden">
          <TouchableOpacity
            onPress={() => toggleSection('budget')}
            className="p-4 flex-row items-center justify-between"
          >
            <View>
              <Text className="text-slate-500 text-sm font-bold uppercase">Total Monthly Budget</Text>
              {expandedSection !== 'budget' && (
                <Text className="text-xl font-bold text-slate-900 mt-1">{currentCurrency}{totalBudget}</Text>
              )}
            </View>
            {expandedSection === 'budget' ? <ChevronUp size={20} color="#64748b" /> : <ChevronDown size={20} color="#64748b" />}
          </TouchableOpacity>
        ...
        <View className="bg-white rounded-2xl border border-slate-100 mb-4 overflow-hidden">
          ...
          >
            <View>
              <Text className="text-slate-500 text-sm font-bold uppercase">Category Allocation</Text>
              {expandedSection !== 'categories' && (
                <Text className="text-slate-900 font-medium mt-1">
                  Allocated: {currentCurrency}{totalAllocated}
                </Text>
              )}
            </View>
            {expandedSection === 'categories' ? <ChevronUp size={20} color="#64748b" /> : <ChevronDown size={20} color="#64748b" />}
          </TouchableOpacity>

          {expandedSection === 'categories' && (
            <View className="p-4 pt-0">
              {CATEGORIES.map((cat) => (
                <View key={cat} className="flex-row items-center justify-between mb-4 last:mb-0 mt-2">
              ...
              <View className="mt-4 pt-4 border-t border-slate-100">
                <View className="flex-row justify-between mb-4">
                  ...
                  <Text className={`font-bold ${
                    ...
                    {currentCurrency}{totalAllocated}
                  </Text>
                </View>
                <TouchableOpacity
        ...
        <View className="bg-white rounded-2xl border border-slate-100 mb-4 overflow-hidden">
          ...
          >
            <View>
              <Text className="text-slate-500 text-sm font-bold uppercase">Currency</Text>
            ...
            {expandedSection === 'currency' ? <ChevronUp size={20} color="#64748b" /> : <ChevronDown size={20} color="#64748b" />}
          </TouchableOpacity>
